...
class CacheManager:
    ...
    def get_policy(self, dispositivo_id: str) -> Optional[Dict[str, Any]]:
        """ Busca no cache uma política de privacidade para o dispositivo. """
        key = f"policy:{dispositivo_id}"
        policy = self.redis_client.get(key)
        if policy:
            print(f"Política de privacidade encontrada no cache para o dispositivo {dispositivo_id}")
            return json.loads(policy)
        print(f"Política de privacidade não encontrada no cache para o dispositivo {dispositivo_id}")
        return None
    
    def set_policy(self, dispositivo_id: str, policy: Dict[str, Any]):
    ...
    def invalidate_policy(self, dispositivo_id: str):
    ...
    def add_data_point(self, dispositivo_id: str, data_point: Any):
        """Adiciona um novo ponto de dado a uma lista para agregação futura."""
        data_key = f"data:{dispositivo_id}"
        self.redis_client.lpush(data_key, json.dumps(data_point))
        print(f"Ponto de dado adicionado para agregação no dispositivo {dispositivo_id}.")

    def get_and_clear_data_points(self, dispositivo_id: str) -> List[Any]:
        """Busca todos os pontos de dados de um dispositivo e limpa a lista."""
        data_key = f"data:{dispositivo_id}"
